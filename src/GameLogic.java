...
public class GameLogic {
    ...
    public void update(double dt) {//проверяет все возможные столкновения
        ...
        for (Player player : players) {
            ...
            if (ballBetween || (players[0].getRect().x + players[0].getRect().width >= players[1].getRect().x))//обработка столкновения игроков
        ...
        if (wait)
            curWaitTime += dt;//реализация задержки мяча
        ...
        if (ball.getPos().x + ball.getRadius() < 10 || ball.getPos().x > 1190) {//реалезация ворот
    ...
    public void movePlayer(int player, double a, boolean jump) {
        if (!jump)
            players[player].setXSpeed(a);//если есть прыжок вызываем метод джамп в плеере
        else
            players[player].jump();
    }
