...
//    }
    ...
    public void move(double dt) {
        ...
        if (!isOnFloor && !isOnBall)
            speed.y += g * dt;//добавляет к вертикальной скорости ускорение
        if (moveBlock * speed.x > 0)//проверяем что снова на полу
            speed.x = 0;
        if (isOnBall)
            speed.y = min(0, speed.y);
        pos.add(new Vector(speed.x * dt, speed.y * dt));//к координатам добавляет скорость
    }
    ...
    public void setMoveBlock(int block) {//устанавливает переданное значение переменной moveblock
        moveBlock = block;
    }
    ...
    public void jump() {
        if (isOnFloor || isOnBall) {//проверяем что на полу
            isOnFloor = false;
            speed.y -= 9;//TODO: remove constant
        }//добавляем скорость по вертикали
    }
}
